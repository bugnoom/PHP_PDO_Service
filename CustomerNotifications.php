...
ini_set("display_errors","1");
    	require_once('class/Customer_Push.php');
	...
	switch($action){
		...
		break;
		case "confirmOrder":
			$sql = "Select fixx_order.id as order_id, fixx_order.*,fixx_user.* FROM `fixx_order` inner join fixx_user on fixx_order.user_id = fixx_user.id WHERE fixx_order.id=:id";
			$arr = array(":id"=>$_REQUEST['id']);
			$data = $service->raw_select_sql($sql,$arr);
			foreach($data as $row){
				$device = $row['device_name'];
